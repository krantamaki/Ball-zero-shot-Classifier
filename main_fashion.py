...
label_dict = {0: "T-shirt", 1: "Trousers", 2: "Pullover", 3: "Dress", 4: "Coat", 5: "Sandal", 6: "Shirt", 7: "Sneaker",
              8: "Bag", 9: "Boot"}
...
def main1():
    ...
    for label in label_dict:
        X.append(sample_to_numpy(get_sample(data, 'label', label, num_points), ['label']))
    ...
    for i in range(len(label_dict)):
        ax.scatter(pca_X[:, 0][i * num_points:(i + 1) * num_points],
                   pca_X[:, 1][i * num_points:(i + 1) * num_points],
                   label=label_dict[i])
    ...
    df_train.to_csv("Data/fashion-mnist_sample_train_100.csv", index=False)
    df_test.to_csv("Data/fashion-mnist_sample_test_100.csv", index=False)


def main2():
    # Read the files with centers and diagonals
    centers = pd.read_csv("Data/fashion_centers.csv")
    diags = pd.read_csv("Data/fashion_diags.csv")

    labels = diags['label'].to_numpy()
    ...
    model.add_diagonals_and_centers(diags, centers, labels)

    # Read test points
    df_test = pd.read_csv("Data/fashion-mnist_sample_test.csv")
    ...
    print(f"\n\nModified SVM testing accuracy: {test_acc}\n")

    # Compare to KNN
    df_train = pd.read_csv("Data/fashion-mnist_sample_train.csv")
    y_train = df_train["label"].to_numpy()

    X_train = df_train.loc[:, ~df_train.columns.isin(["label"])].to_numpy()
    ...
    print(f"K-Neighbors testing accuracy: {len(correct_classifications) / y_test.shape[0]}")
...
def main3():
    ...
    vectors = np.array([model.get_vector(f"{label_dict[label].lower()}_NOUN") for label in label_dict])
    """
    # Define semantic vectors using PCA
    data = pd.read_csv("Data/fashion-mnist_train.csv", sep=',')
    vect_dict = gen_semantic_vectors(data, 100, method='PCA')

    print(vect_dict)

    # Get the data
    df_train = pd.read_csv("Data/fashion-mnist_sample_train_100.csv")

    df_test = pd.read_csv("Data/fashion-mnist_sample_test_100.csv")
    y_test = df_test["label"].to_numpy()
    X_test = df_test.loc[:, ~df_test.columns.isin(["label"])].to_numpy()
    ...
    for i in range(0, 4 + 1):
        # Read the files with centers and diagonals
        centers = pd.read_csv(f"Data/fashion_centers_{i}_left_out_100.csv")
        diags = pd.read_csv(f"Data/fashion_diags_{i}_left_out_100.csv")
        ...
        model.add_sematic_vectors(rescale(np.array(list(vect_dict.values())), 10**15), np.array(list(vect_dict.keys())))
